...
import ch.emineo.roo.reverse.web.MessageController;
...
privileged aspect MessageController_Roo_Controller {
    ...
    public String MessageController.create(@Valid Message message, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        ...
        uiModel.asMap().clear();
        message.persist();
        return "redirect:/messages/" + encodeUrlPathSegment(message.getMessageid().toString(), httpServletRequest);
    }
    ...
    void MessageController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("message_creationtime_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
